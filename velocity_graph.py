...
from scvelo.tl import velocity
    ...
    def _compute_cosines(self, obs_idx, queue):
        ...
        for obs_id in obs_idx:
            if self.V[obs_id].max() != 0 or self.V[obs_id].min() != 0:
                ...
                # lvpt transition matrix
                sigma = np.mean(np.linalg.norm(dX,axis=1))
                normpdf = stats.norm(scale = sigma).pdf(np.linalg.norm(dX,axis=1))
                val = self.steady * normpdf + val * normpdf
