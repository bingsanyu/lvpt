...
def velocity_map(adata=None, T=None, n_dcs=10, return_model=False):
    lvpt = LVPT(adata, n_dcs=n_dcs)
    if T is None:
        T = adata.uns["velocity_graph"] - adata.uns["velocity_graph_neg"]
        lvpt._connectivities = T + T.T
    lvpt.compute_transitions()
    lvpt.compute_eigen(n_dcs)
    adata.obsm["X_vmap"] = vpt.eigen_basis
    return lvpt if return_model else None
...
def velocity_pseudotime(
    ...
    **kwargs:
    ...
    for cat in categories:
        ...
        lvpt = LVPT(data, n_dcs=n_dcs, **kwargs)

        if use_velocity_graph:
            T = data.uns[f"{vkey}_graph"] - data.uns[f"{vkey}_graph_neg"]
            lvpt._connectivities = T + T.T

        # vpt._connectivities=my_trans

        lvpt.compute_transitions(density_normalize=True)
        lvpt.compute_eigen(n_comps=n_dcs)

        lvpt.set_iroot(root_key)
        lvpt.compute_pseudotime()
        dpt_root = lvpt.pseudotime

        if end_key is not None:
            lvpt.set_iroot(end_key)
            lvpt.compute_pseudotime(inverse=True)
            dpt_end = lvpt.pseudotime

            # merge dpt_root and inverse dpt_end together
            lvpt.pseudotime = np.nan_to_num(dpt_root) + np.nan_to_num(dpt_end)
            lvpt.pseudotime[np.isfinite(dpt_root) & np.isfinite(dpt_end)] /= 2
            lvpt.pseudotime = scale(lvpt.pseudotime)
            lvpt.pseudotime[np.isnan(dpt_root) & np.isnan(dpt_end)] = np.nan

        if "n_branchings" in kwargs and kwargs["n_branchings"] > 0:
            lvpt.branchings_segments()
        else:
            lvpt.indices = lvpt.pseudotime.argsort()
